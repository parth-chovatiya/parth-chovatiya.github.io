...
function fetchBlogsFromMedium() {
  fetch(
    "https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/feed/@parthchovatiya"
  )
...
function populateBlogs(items, id, subid) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    img.src =
      items[i].thumbnail ||
      items[i].description.match(/<img[^>]+src="([^">]+)"/)[1];
    img.className = "img-fluid";
...
function populateExp_Edu(items, id) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    h2TimelineLabel.style.fontWeight = "bold";
    ...
    for (let j = 0; j < items[i].tags.length; j++) {
      ...
      spanTags.className = "badge badge-secondary edu-badge";
